"""
Bimanual IK solver with symmetric rest pose for consistent arm configurations.
"""
...
def solve_ik_bimanual_symmetric(
    robot: pk.Robot,
    target_link_names: Sequence[str],
    target_wxyzs: onp.ndarray,
    target_positions: onp.ndarray,
    rest_pose: onp.ndarray | None = None,
    rest_weight: float = 1.0,
) -> onp.ndarray:
    """
    Solves bimanual IK with optional rest pose for symmetric configurations.
    ...
    Args:
        ...
        rest_pose: onp.ndarray. Shape: (num_joints,). Preferred joint configuration.
                   If None, uses all zeros.
        rest_weight: float. Weight for rest pose cost (higher = stronger bias).
    ...
    if rest_pose is None:
        rest_pose = onp.zeros(robot.joints.num_actuated_joints)
    else:
        assert rest_pose.shape == (robot.joints.num_actuated_joints,)
